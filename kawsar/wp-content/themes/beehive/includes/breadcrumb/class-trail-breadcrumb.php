	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches ) ? sprintf( '%s<span itemprop="name">%s</span>%s<meta itemprop="position" content="%d">', $matches[1], $matches[2], $matches[3], $item_position ) : sprintf( '%s', $item );
				...
				$attributes = ! empty( $matches ) ? 'itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem" class="' . $item_class . '"' : 'class="' . $item_class . '"';
