...
if ( ! class_exists( 'CMB2_Tabs', false ) ) {
	...
	class CMB2_Tabs {
		 ...
		 * @param int    $object_id     Metabox object ID.
		 * @param string $object_type   Metabox object type.
		 * @param object $cmb           The object.
		 *
		 * @since 1.0.0
		 */
		...
		public function render_nav( $cmb_id, $object_id, $object_type, $cmb ) {
			...
			if ( $tabs ) {
				...
				foreach ( $tabs as $key => $tab_data ) {
					...
					printf( '<li class="%s" data-panel="%s"><a href="#">%s<span>%s</span></a></li>', $class, $key, $icon, $tab_data['label'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
		 ...
		 *
		 * @param array  $classes      Panel wrapper classes.
		...
		public static function tabs_render_row_cb( $field_args, $field ) {
			...
			if ( 'group' === $field_args['type'] ) {
				...
				if ( $field->args( 'cmb2_tabs_render_row_cb' ) ) {
					self::$cmb->peform_param_callback( 'cmb2_tabs_render_row_cb' );
				} else {
		 ...
		 *
		 * @param array  $field_args    Field arguments.
		 * @param object $field_group   Field object.
		 *
		 * @since 1.0.5
		 */
		public static function tabs_render_group_row_cb( $field_args, $field_group ) {
			...
			if ( $field_group->args( 'cmb2_tabs_render_row_cb' ) ) {
				self::$cmb->render_group_callback( 'cmb2_tabs_render_row_cb' );
			} else {
				self::$cmb->render_group_callback( $field_args, $field_group );
			}
		...
		public function show_panels( $cmb_id, $object_id, $object_type, $cmb ) {
			...
			foreach ( $this->fields_output as $tab => $fields ) {
				if ( ! in_array( $tab, $this->conditional, true ) ) {
					...
					echo '<div class="' . esc_attr( $active_panel ) . ' cmb-tab-panel cmb-tab-panel-' . esc_attr( $tab ) . '">';
					echo implode( '', $fields ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '</div>';
				}
			}
		...
		public function setup_admin_scripts() {
			...
			wp_register_script( 'cmb-tabs-js', self::url( 'js/tabs.js' ), array( 'jquery' ), self::VERSION, true );
		 ...
		 *
		 * @param string $cb    Show if front page.
		 *
		 * @since 1.0.0
		 */
