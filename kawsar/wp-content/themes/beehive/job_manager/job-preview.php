...
<form method="post" id="job_preview" action="<?php echo esc_url( $form->get_action() ); ?>">
	...
	<div class="job_listing_preview_title">
		<h4 class="preview-title"><?php esc_html_e( 'Preview', 'wp-job-manager' ); ?></h4>
		<input type="submit" name="edit_job" class="button job-manager-button-edit-listing button-solid" value="<?php esc_attr_e( 'Edit listing', 'wp-job-manager' ); ?>" />
		<input type="submit" name="continue" id="job_preview_submit_button" class="button job-manager-button-submit-listing button-solid" value="<?php echo esc_attr( apply_filters( 'submit_job_step_preview_submit_text', esc_attr__( 'Submit Listing', 'wp-job-manager' ) ) ); ?>" />
	</div>
